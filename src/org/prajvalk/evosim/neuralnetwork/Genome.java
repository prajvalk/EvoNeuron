...
public class Genome {
    ...
    Genome(int in, int out) {
        ...
        for (int i = 0; i < in; i++) {
            ...
            for (int j = 0; j < outputs; j++) {
                ...
                genes.add(new ConnectionGene(nodes.get(i), nodes.get(inputs + j), random(-1, 1), localNextConnectionNumber));
        ...
        //connect the bias
        for (int i = 0; i < outputs; i++) {
            genes.add(new ConnectionGene(nodes.get(biasNode), nodes.get(inputs +i), random(-1, 1), localNextConnectionNumber));
            localNextConnectionNumber  ++;
        }
    }
    ...
    void addNode(ArrayList<ConnectionHistory> innovationHistory) {
        ...
        int randomConnection = (int) floor(random(genes.size()));

        while (genes.get(randomConnection).fromNode == nodes.get(biasNode)) {//dont disconnect bias
            randomConnection = (int) floor(random(genes.size()));
        }
    ...
    void addConnection(ArrayList<ConnectionHistory> innovationHistory) {
        ...
        //get random nodes
        int randomNode1 = (int) floor(random(nodes.size()));
        int randomNode2 = (int) floor(random(nodes.size()));
        while (nodes.get(randomNode1).layer == nodes.get(randomNode2).layer
            ...
            //get new ones
            randomNode1 = (int) floor(random(nodes.size()));
            randomNode2 = (int) floor(random(nodes.size()));
        }
        int temp;
    ...
    int getInnovationNumber(ArrayList<ConnectionHistory> innovationHistory, Node from, Node to) {
        boolean isNew = true;
        int connectionInnovationNumber = Constants.nextConnectionNo;
    ...
    public float random(int maxrange) {
        return ThreadLocalRandom.current().nextFloat() * maxrange;
    }

    public float random(int lowerBound, int upperBound) {
        if(lowerBound < 0) {
            if(ThreadLocalRandom.current().nextBoolean()) {
                return lowerBound + ThreadLocalRandom.current().nextFloat() * (upperBound - lowerBound);
            } else {
                return -( lowerBound + ThreadLocalRandom.current().nextFloat() * (upperBound - lowerBound) );
            }
        } else {
    ...
    public void println(String... args) {
        ...
        for(String a:args) inp.append(a);
